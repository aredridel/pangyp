  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      req.on('close', function () {
        if (extractCount === 0) {
          cb(new Error('Connection closed while downloading tarball file "%s"', tarballUrl))
        }
      })
      ...
      req.on('response', function (res) {
        if (res.statusCode !== 200) {
          badDownload = true
          cb(new VError("%s status code downloading tarball %s", res.statusCode, tarballUrl));
          return
        }
        // content checksum
      ...
      function afterTarball () {
        ...
        if (extractCount === 0) {
          return cb(new VError('There was a fatal problem while downloading/extracting the tarball "%s"', tarballUrl))
        }
